<template>
  <div v-if="socialStore.activeChatId" 
       class="fixed right-80 bottom-0 w-80 h-96 bg-[#151515] border border-white/10 rounded-t-xl shadow-2xl flex flex-col z-50 overflow-hidden transform transition-all duration-300">
    
    <!-- Title Bar -->
    <div class="h-10 bg-[#222] flex items-center justify-between px-3 border-b border-white/5">
        <div class="flex items-center gap-2 cursor-pointer" @click="socialStore.closeChat()">
            <div class="w-2 h-2 rounded-full" 
                 :class="activeFriend?.status === 'ONLINE' ? 'bg-green-500' : 'bg-zinc-500'"></div>
            <span class="text-xs font-bold text-white">{{ activeFriend?.pseudo || 'Chat' }}</span>
        </div>
        ...
        <div class="flex items-center gap-1">
            ...
            <button @click="socialStore.closeChat()" class="text-zinc-500 hover:text-white p-1">✕</button>
        </div>
    </div>

    <!-- Messages -->
    <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar flex flex-col-reverse">
    ...
    <!-- Input -->
    <div class="p-2 bg-[#222] border-t border-white/5">
